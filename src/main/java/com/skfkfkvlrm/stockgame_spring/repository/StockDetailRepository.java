...
public interface StockDetailRepository {
    ...
    List<Order> getTotalOrder(int stockNo);
    // 특정 주식 대기중인 매도 주문 모두 조회
    List<Order> getTotalSellOrder(int stockNo);
    // 특정 주식 대기중인 매수 주문 모두 조회
    List<Order> getTotalBuyOrder(int stockNo);
    // 내 주문 요청 조회
    List<Order> getTotalMyOrder(int stockNo, String studentId);
    // 직전에 등록한 주문요청 no 조회
